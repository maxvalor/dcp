...
void TCPConnection::ReadHeader()
{
    ...
    boost::asio::async_read(
        *sock, boost::asio::buffer(header.get(), Message::SIZE_OF_HEADER),
        std::bind(&TCPConnection::OnHeaderReceived, this, self, header, std::placeholders::_1, std::placeholders::_2));
}


void TCPConnection::ReadBody(std::shared_ptr<Message::Header> header) {

    DLOG(INFO) << "ReadBody";

    auto self = shared_from_this();

    auto msg = Message::Generate(header);
...
void TCPConnection::OnHeaderReceived(std::shared_ptr<TCPConnection> self,
        std::shared_ptr<Message::Header> header,
        boost::system::error_code ec, std::size_t receivedLength)
{
//    DLOG(INFO) << "Read header from remote, size: " << receivedLength;

    if (ec != boost::system::errc::success
            || Message::SIZE_OF_HEADER != receivedLength)
    {
        Stop();
        DLOG(ERROR) << "an exception occurred after receive message: " << ec;
    ...
    else
    {
        ...
        ReadHeader();
        funcMessageReceived(Message::Generate(header));
    }
}
...
void TCPConnection::PendMessage(std::shared_ptr<Message> msg)
{
    {
...
void TCPConnection::Send(std::shared_ptr<Message> msg)
{
    ...
    if(Send((std::uint8_t*)&header, Message::SIZE_OF_HEADER)
